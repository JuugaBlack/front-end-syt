<template>
  ...
  <el-card style="max-width:100%" class="box-card">
    ...
    <div class="tip">
      ...
      <p>完成实名认证后才能添加就诊人，为了不影响后续步骤，建议提前实名认证。</p>
    </div>
    ...
    <el-descriptions :column="1" border v-if="userInfo.authStatus==1">
      ...
      <el-descriptions-item label="证件类型">{{ userInfo.certificatesType == '10' ? '身份证' : '户口本' }}</el-descriptions-item>
      <el-descriptions-item label="证件号码">{{ userInfo.certificatesNo }}</el-descriptions-item>
    </el-descriptions>
    <!-- 未验证 v-if="userInfo.authStatus==0"-->
    <el-form  :model="params" :rules="rules"  v-if="userInfo.authStatus==0" style="width: 80%;margin: 20px auto" label-width="80">
      ...
      <el-form-item label="上传证件" prop="certificatesUrl" ref="form">
        ...
        <el-dialog v-model="dialogVisible">
          <img w-full  v-if="params.certificatesUrl" :src="params.certificatesUrl" style="width: 100%;height: 100%;" alt="Preview Image" />
        </el-dialog>
      </el-form-item>
      <el-form-item>
...
import { reqCertificationType, reqUserCertification, reqUserInfo } from '@/api/user';
...
const reset = () =>{
  ...
  Object.assign(params,{certificatesNo: '',certificatesType: '',certificatesUrl: '',name: '',})
}
